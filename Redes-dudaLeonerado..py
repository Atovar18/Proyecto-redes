...
l = np.array(Z["L (uH)"])
historia_induc.extend(1j * w * l)

c = np.array(Z["C (uF)"])
historia_capa.extend(-1j / (w * c))
...
for i in range(len(bus_i)):
    for h in range(i + 1, len(bus_j)):
        print ((range(i, len(bus_i))), "la z")
        print ((range (h, (len (bus_j)))), "la h")
        for z in range (z, len(bus_j)):
             for b in range (z+1, len(bus_j)):
                if bus_i[i] == bus_i[h] and bus_j[z] == bus_j[b]:
                    # Calculamos las equivalentes en paralelo
                    req = 1 / (1 / historia_resis[i] + 1 / historia_resis[b])
                    leq = 1 / (1 / historia_induc[i] + 1 / historia_induc[b])
                    ceq = 1 / (1 / historia_capa[i] + 1 / historia_capa[b])
                    # Eliminamos los elementos originales de las listas
                    del historia_resis[h]
                    del historia_induc[h]
                    del historia_capa[h]
                    del historia_resis[i]
                    del historia_induc[i]
                    del historia_capa[i]
                    # Insertamos los elementos equivalentes en las listas
                    historia_resis.insert(i, req)
                    historia_induc.insert(i, leq)
                    historia_capa.insert(i, ceq)
                    # Actualizamos los índices de bus_i y bus_j
                    bus_i = np.delete(bus_i, [i,j])
                    bus_j = np.delete(bus_j, [i,j])
                    bus_i = np.insert(bus_i, i, bus_i[i])
                    bus_j = np.insert(bus_j, i, bus_j[i])
            
# Imprimimos los resultados
print("Resistencias equivalentes en paralelo:", historia_resis)
print("Inductancias equivalentes en paralelo:", historia_induc)
print("Capacitancias equivalentes en paralelo:", historia_capa)
...
if nodos[0] == 0:
    c = len(nodos) -1
    print ("La matriz tiene tamaño 5", c)
elif nodos [0] != 0:
     c = len(nodos)
