...
for cap in capacitores:
    if cap == 0:                                             #Analizamos los datos de la lista para que el código no realice la división si el capacitor = 0
        ...
        ListZcap.extend (list(Zcap))
    ListZcap.append(round(Zcap, 4))
...
#for cap in datoscapacitores_I:
          ...
          #Zcapi = 0
     #else:
          #Zcapi = (-1/(w*(cap*(10**-6)))*1j)                    #Calculamos las impedancias de los capacitores.
#ListZcap_I.extend (list(Zcapi))
#ListZcap_I.append(round(Zcapi, 4))                                        #Guardamos en una lista.

datosinductores_I = np.array (I_fuente ["Lf (mH)"])        #Selecionamos los inductores y calculamos sus imperancias. 
Zinduc_I = (w * (datosinductores_I)*(10**-6))*1j
...
for gen in Zgen:
     if gen == 0:
        AdmitanciasGenV.append(0)  
     else:
        AdmitanciasGenV.append(1 / gen)

AdmitanciasGenV = np.array(AdmitanciasGenV)
AdmitanciasGenV = np.round (AdmitanciasGenV , 4)
Iinyectadas = np.divide (SeriesFv,gen)
...
Z_eq = np.sum((historia_capa, historia_induc, historia_resis), axis = 0)

Z_eq_sin_cero = np.where(Z_eq == 0, 1e+17, Z_eq)

print(Z_eq, "Puta fisica")
Y_eq = 1 / Z_eq_sin_cero
...
print (bus_j, "busj")
result = {}
for i, j, z in zip(bus_i, bus_j, Y_eq):
    if i in result:
        result[i-1] += z
    else:
        result[i-1] = z
print(result, "La puta madre andradeeeeeeeee")
...
def matriz_admittancia_Z(Y_eqq, bus__i, bus__j):       # Definimos la matriz de admittancia nodal
    ...
    for i in range(len(Y_eq)):
        ...
        Y_Z[bus__j, bus__j] = np.sum((Y_eqq, result.get(i, 0)), axis = 0) 
    return Y_Z
...
Y_bus = np.sum((Y_Z, Admitt_V), axis = 0)
print("Ybus")
print (Y_bus)
